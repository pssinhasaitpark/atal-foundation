...
const OpSpecialSupport = () => {
  ...
  let specialSupportData = [];
  if (!isLoading && !isError) {
    specialSupportData = data.filter(
      (item) => item.category === "Special Support ourProgramme"
    );
  }
  if (isLoading) return <div className="spinner"></div>;
  if (status === "failed") return <div>Error: {isError}</div>;
  const dynamicData = specialSupportData[0].details;
  const bannerImage = specialSupportData[0].banner;
...
export default OpSpecialSupport;
